...
\usepackage{esvect} %李浩林添加用于矢量箭头 \vv{AB}
\usepackage{harpoon} %李浩林添加用于矢量箭头  \overrightharp
\usepackage{overpic}%李浩林添加用于在插入的图片上添加文字说明
\usepackage{array}%李浩林添加用于设置表格宽度
\usepackage{ulem}%李浩林添加用于输入短横线
\usepackage{listings} %李浩林添加 用于显示代码
\usepackage{tikz} %李浩林添加用于绘制简图
\usepackage{tikz-3dplot} %李浩林添加用于绘制简图
\usetikzlibrary{datavisualization.formats.functions} % add by jtyu for visualization 
%\usepackage{algorithm}
\usepackage{algorithmic}
\usepackage{minibox}
% 支持绘制UML
\usepackage[simplified]{pgf-umlcd}  %hlli添加用于绘制UML类图，添加simplified选项，删除不必要的空格
%\usepackage{pgf-umlcd-gn}  %hlli添加用于绘制UML类图，添加simplified选项，删除不必要的空格
% 其他
